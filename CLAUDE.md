...
| Prefix | 용도 | 작성 시점 |
|---|---|---|
| `[요구사항]` | 무엇을 만들어야 하는지 (기능·조건·제약) | 프로그래밍 작업 지시 시 코드 작성 전 |
| `[현상분석]` | 현행 구조·원인·영향 범위 분석 | 프로그래밍 작업 지시 시 코드 작성 전 |
| `[구현계획]` | 어떻게 구현할지 (설계·단계·파일 변경 계획) | 프로그래밍 작업 지시 시 코드 작성 전 |
...
**프로그래밍 작업 흐름**: 작업 지시 → `[요구사항]`/`[현상분석]`/`[구현계획]` 문서 작성 → 사용자 확인 → 승인 시 코드 구현.

예시: `[요구사항] LED 제어.md`, `[현상분석] LED 운용 방식.md`, `[구현계획] LED 제어.md`

컨벤션은 강제 규칙이 아니라 가이드. 다른 prefix(`[리뷰]`, `[참고]` 등)도 자유롭게 사용 가능.

## 네이밍 컨벤션
C 코드의 변수·함수·매크로·타입·파일 네이밍 규칙. 상세 내용은 [`docs/[지침] 네이밍 컨벤션.md`](docs/[지침]%20네이밍%20컨벤션.md) 참조.

## 문서 작성 기본 포멧
**기본값**: 사용자가 명시적으로 다른 포멧(docx / xlsx / pdf / pptx / html 등)을 지정하지 않는 한, **모든 문서는 Markdown (`.md`)로 작성한다.** 예외 없음.

- 적용 범위: 일반 문서, 노트, 기획서, 설명서, 리포트, README, SW 문서(`[요구사항]`/`[현상분석]`/`[구현계획]`/`[시험방법]`/`[사용방법]` 등) 모두 포함

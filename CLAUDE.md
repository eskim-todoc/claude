...
- **세션 종료 · compact 직전 · "요약해줘"** 시 프로젝트 `docs/세션_인수인계/` 에 요약 작성. 파일명 `YYYYMMDD_HHmm_<주제>.md`
- **구간은 대화 컨텍스트가 아니라 git 으로 잡는다** — 직전 요약 문서가 **커밋된 지점**부터 `HEAD` 까지. compact 가 두 번 이상이면 컨텍스트에서 경계가 사라진다
  ```sh
  PREV=$(git log --format=%H --reverse -- "docs/세션_인수인계/<직전문서>.md" | head -1)
  git log $PREV..HEAD --format=%B
  ```
- 문서에 «작성 시점 HEAD» 를 적지 말 것 — **자기참조**(커밋 순간 한 칸 뒤처진다)
- 담을 것: **구간 · 지금 상태 · 한 일(병합 해시) · 자산 · 남은 것(왜 막혔는지) · 권장 순서**. **자산**이 존재 이유
- **전제**: 커밋 메시지에 «왜 그렇게 했는지»를 적어야 `git log` 재구성이 성립. 접은 대안·구두 지시는 요약 본문이 담는다
- 상세: [`지침/일반/세션 인수인계 규칙.md`](지침/일반/세션%20인수인계%20규칙.md)

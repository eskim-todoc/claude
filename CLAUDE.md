...
> [!IMPORTANT]
> `maturity: core` 핵심 규칙의 초압축 인라인. 하네스가 세션마다 이 파일 전문을 자동 주입하므로 링크를 안 타도 즉시 효력 — 위반 시 즉시 중단. 상세·절차·예외는 각 `지침/` 파일(명시 Read), **전체 지침 인덱스는 [`지침/README.md`](지침/README.md)가 단일 출처**.
...
- 상세: [`지침/서브에이전트/활용.md`](지침/서브에이전트/활용.md), [`지침/서브에이전트/오케스트레이션.md`](지침/서브에이전트/오케스트레이션.md)
...
## 지침 문서 (전체)
전체 지침 파일 인덱스(maturity·목적·사용 시점)는 **[`지침/README.md`](지침/README.md)가 단일 출처(SSOT)**다. CLAUDE.md는 위 §핵심 지침으로 core 규칙만 인라인하고, 전체 목록은 중복 유지하지 않는다(R1 이중 인덱스 drift 방지).

> [!NOTE]
> **2026-05-18 Package C**: `docs/회고/` 폐지 → 회고 항목을 각 지침 §학습 노트로 흡수(maturity 라벨). 신규 학습은 해당 지침 §학습 노트에 `experimental`로 추가 → 검증 후 승급.

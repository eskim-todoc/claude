...
>
> **기억 회수 (무조건)**: [`기억/README.md`](기억/README.md)를 `Read` 한다. **인덱스만** 보고, 지금 하는 일과 관련된 줄이 있을 때만 본문 파일을 연다. **링크가 걸려 있다는 것만으로는 회수되지 않는다** — 이 «Read 하라»는 지시가 회수의 유일한 보장이다(반례: `.claude/memory/preferences.md`는 아래 §Memory에 링크돼 있었으나 아무도 읽지 않아 잠들어 있었다).
...
모든 작업은 다음 흐름으로 진행 (단순 수정 면제):
- ① 인터뷰·`요구사항.md` → ② 분석·`계획.md` → **[게이트 1] 계획서 체크박스** → ③ `검증리스트.md` → ④ 구현·자체 선테스트 → **[게이트 2] 검증리스트 체크**
- **검증은 은수님 체크박스다** — Claude 자체 점검 문서가 게이트를 대신하지 않음. 게이트는 **2회뿐**
- ① **인터뷰**: 모호점 0 + 수용 기준을 쓸 수 있을 때까지 질문. 그 기록이 곧 요구사항 검증 (별도 검증 문서 없음)
- ② 계획서가 **분석을 흡수**. 사실 주장엔 `파일:라인` 근거 · **실패 시나리오 ≥1 필수** · 전수 영향 조사는 **명칭·산출물 파일명·단계 번호·양식 링크 4축**
- ③ 검증리스트는 **Claude 열/은수님 열 2열** — 기계적 확인은 구현 직후 Claude가 채우고, 써 봐야 아는 것만 은수님께
- 구현 중 **계획 결함** 드러나면 중단 + **`이슈보고서_N_<주제>.md`** (오타·링크 오류는 고치고 진행)
...
- **문서화 필수** — **2노드+ fan-out 시 `docs/tasks/<작업>/에이전트-로그/` 필수**: 오케스트레이터 입력·위임 설계·프롬프트(`00_오케스트레이터-입력.md`) + 각 노드 결과(`NN_<역할>.md`) + `인덱스.md`. 4단계 산출물도 정규 문서로. **쓰기 fan-out은 게이트 1 통과 후에만**(그 전엔 read-only만). **위임 프롬프트에 "결과를 로그 파일에 직접 Write하라"를 명시**(누락 재발 방지)
...
- 코드는 단계 ④ — 요구사항·계획 산출물(`요구사항.md`/`계획.md`) + **게이트 1 통과** 전 작성 금지
- 신규 심볼·모듈은 `tdc_` 접두어
...
- **규칙은 `지침/`, 사실은 [`기억/`](기억/README.md)** — "이렇게 해라"와 "실제로 이렇다"를 섞지 않는다
- **회수**: 세션 시작 시 [`기억/README.md`](기억/README.md) 인덱스 Read (위 §세션 시작 첫 동작). **기록**: 은수님 **「기억해」** → 무조건 · Claude 판단 → **여쭙고 승낙 시**
- **갱신이 본질** — 새 파일보다 **기존 파일 갱신 우선**. 확신도 승급(추정→확인됨→실측 확인) · **정정 이력 보존**(틀렸던 것을 지우지 않는다) · 기록할 때마다 **즉시 커밋**(푸시는 은수님)
- 상세: [`기억/운영 규약.md`](기억/운영%20규약.md), 아래 §Memory
...
> [!IMPORTANT]
> **Claude Code 글로벌 auto-memory 시스템(`~/.claude/projects/<프로젝트>/memory/`) 사용 금지.** 장비 로컬이라 git·백업이 닿지 않고, 경로가 프로젝트 **절대경로**에 묶여 있어 폴더를 옮기면 통째로 사라진다. 시스템 프롬프트의 auto-memory 안내는 이 프로젝트에서 비활성화.

**기억은 git tracked 로 관리하되, 「규칙」과 「사실」을 나눈다** (2026-08-18 개정 — 이전에는 *"모든 학습은 `지침/` §학습 노트로"* 였으나, 규칙이 아닌 사실을 둘 곳이 없어 인수인계 문서·참고 문서·대장 세 곳에 흩어졌다).

| 성격 | 위치 | 예 |
|---|---|---|
| **규칙** — "이렇게 해라" | [`지침/`](지침/) 및 각 파일 `§학습 노트` | 4단계 2게이트 프로세스 · 커밋 규약 · 게이트 규칙 |
| **사실** — "실제로 이렇다" | **[`기억/`](기억/README.md)** (2026-08-18 신설) | 은수님 업무 방식 · 제품·시험·하드웨어의 확정 사실 |

- **회수**: 세션 시작 시 [`기억/README.md`](기억/README.md) **인덱스를 Read**(위 §세션 시작 첫 동작). 관련될 때만 본문
- **기록**: 은수님 **「기억해」** → 무조건 · Claude 판단 → **여쭙고 승낙 시**. 임의 생성 금지
- **갱신**: 새 파일보다 **기존 파일 갱신 우선**. 확신도 승급 · 정정 이력 보존 · 기록 즉시 커밋(**푸시는 은수님**)
- 상세: [`기억/운영 규약.md`](기억/운영%20규약.md)

`.claude/memory/`는 이 체계 **이전**의 것 — git tracked, 수동 관리. 링크만 있고 «읽어라»가 없어 사실상 잠들어 있었다. **`기억/`으로 흡수할지는 은수님 판단**([`기억/README.md`](기억/README.md) §미결):

...
**TL;DR**: actionable task 시작 시 Claude가 `EnterPlanMode`를 **자체 호출**해 Plan Mode 진입 → 표준 경로에 ②분석·③계획 산출물 정식 작성 → `ExitPlanMode`로 사용자 승인 게이트 통과 → Auto 모드로 ④구현. 표준 4단계 산출물은 100% 보존. 단순 수정·조회는 면제.
...
flowchart TD
    A["사용자 명령"] --> B{모호?}
    B -->|예| C["명령 해석 규칙<br/>인터뷰/일괄 질문"]
    B -->|아니오| D{면제 케이스?}
    C --> D
    D -->|예| E["Auto 모드 바로 진행"]
    D -->|아니오| F["①요구사항.md 작성"]
    F --> G["EnterPlanMode 호출"]
    G -->|승인 1차| H["Plan Mode 내<br/>②분석.md ③계획.md<br/>표준 경로에 작성"]
    H --> I["ExitPlanMode 호출"]
...
## 4. Plan Mode plan file vs 표준 산출물
...
### 4.1 plan file 저장 위치 (`plansDirectory`)
...
- **Git 추적**: 저장 폴더는 `.gitignore`로 추적 차단 (`docs/plans/*` + `!docs/plans/.gitkeep`) — 폴더 구조만 유지, plan file 자체는 세션 임시
...
### 5.1 EnterPlanMode 호출 시점

- ①요구사항 명확화 완료 직후, ②분석 시작 전
...
### 5.3 ExitPlanMode 호출 전 self-check
...
1. 본 규칙은 [`작업 진행 규칙.md`](작업%20진행%20규칙.md)의 4단계 위에서 작동 — 4단계 면제 케이스면 본 규칙도 면제
2. [`명령 해석 규칙.md`](명령%20해석%20규칙.md) < 본 규칙 (모호 해소 후 Plan Mode 진입)
3. [`컨텍스트 절약 규칙.md`](컨텍스트%20절약%20규칙.md) < 본 규칙 (승인 게이트가 잘못된 방향 구현보다 저렴)

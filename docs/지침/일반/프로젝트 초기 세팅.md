...
flowchart TD
    ...
    D --> E["③ 표준 폴더 풀 세트<br/>src·tests·docs/지침·docs/tasks/작업 목록.md"]
...
> [!NOTE]
> `사용방법/`·`참고/` 폴더는 표준에서 **제외**. 도구·CLI 매뉴얼·외부 스펙·도메인 지식은 Wiki Vault(`projects/wiki/`, 신규 프로젝트라면 해당 도메인 폴더를 새로 만들거나 기존 도메인에 편입)에 직접 작성한다 (영속 콘텐츠 단일 출처 정책, Task α 결정. 2026-07-08 도메인 재편 이후 배치 기준은 [`projects/wiki/README.md`](../../../projects/wiki/README.md) 참조).
...
> - 본 프로젝트 `docs/지침/` ↔ 루트 `docs/지침/` 충돌 시 → 루트 [`문서 작성 규칙.md §1.1`](../../docs/지침/일반/문서%20작성%20규칙.md)에 따라 **프로젝트 우선** (의도적 도메인 특화 예외).
...
- 컨벤션 상세: 루트 [`지침/일반/문서/폴더-구조.md`](../../../../docs/지침/일반/문서/폴더-구조.md) §5

...
flowchart TD
    ...
    E --> F["⑥ 종료 정리 (현 구조)<br/>이력.md + _archive 이동 + list.md 완료 + _modules.md 자기 반영"]
```
...
1. `이력.md` final 요약 섹션 작성 (시계열 로그는 진행 중 누적됨)
2. `tasks/list.md` 해당 행의 `상태` 컬럼을 `완료`로 변경 + `완료일` 기록
3. `tasks/_modules.md` 갱신 — 해당 모듈 행의 `최종 업데이트`(완료일)·`최근 작업`(본 작업 폴더 링크)·`작업 수`(+1) 갱신. 신규 모듈이면 행 추가. 정렬 재조정. 본 인덱스는 Wiki 등 외부 sync 진입판단용
...
3. **`_modules.md` 갱신** — 모듈 인덱스에 작업 반영 ([`작업 진행 규칙.md §6.1`](작업%20진행%20규칙.md))
4. **커밋 → 사용자 승인 → `--no-ff` 병합**
```
...
└── tasks/                          작업 스코프
    ...
    ├── _modules.md                 모듈 인덱스 (모든 작업 누적)
    └── <모듈>/                      작업 폴더 (활성·완료 무관 영구 위치)
        └── <작업슬러그>/
            ├── 요구사항.md
            ├── 분석.md
            ├── 계획.md
            └── 이력.md             (시계열 로그 + 완료 시 요약 추가)
```
...
```markdown
- [ ] 작업 종료 시 `이력.md` 완료 요약 추가 + `list.md` 상태 변경(완료) + `_modules.md` 갱신
```
...
```markdown
> [!NOTE]
> `_modules.md`는 첫 작업 종료 시점에 [`작업 진행 규칙.md §6.1`](작업%20진행%20규칙.md) 3번 단계가 자동 생성. 신규 프로젝트 초기에 미리 만들지 않음.
```

(기존 "_archive/ 폴더와 _archive/_modules.md" 표현 → "_modules.md"로 단순화)
...
| 이력 | `이력.md` | 의무. **단계 ① 시작과 동시에 생성** — 시계열 로그(진행 중 누적) + 완료 요약(종료 시 추가) |
```

(기존 `진행상황.md` 행 삭제, 이력.md 비고에 "단계 ① 시작과 동시에 생성" 추가)

#### `작업 진행 규칙.md §3` 진행상황 문서 절 폐지

§3 전체 → `이력.md`에 흡수. 새 §3 본문:
...
```markdown
- 필수 파일: `요구사항.md`, `분석.md`, `계획.md`, `이력.md` (단계 ① 시작과 동시에 생성, 시계열 로그 + 완료 요약 통합 — [`작업 진행 규칙.md §3`](작업%20진행%20규칙.md))
```

(기존 `진행상황.md` 행 삭제)

#### `문서 작성 규칙.md §3.1` 트리 (위 §2.1에서 이미 반영 — 이력.md 통합)

#### `작업 규칙.md` 마스터 L47

```markdown
- [ ] `이력.md`를 단계 ① 시작과 동시에 생성하고 단계 전이·결정·이슈마다 시계열 로그 갱신, 완료 시 요약 추가
```
...
| `이력.md` 갱신 | 해당 행의 `상태`·`요약` 업데이트 |
| 사용자 `작업 끝났어. 마무리해` 시그널 | (1) `이력.md` 완료 요약 추가 → (2) `list.md` 행의 `상태`를 `완료`로 변경·`완료일` 기록 → (3) `_modules.md` 모듈 행 갱신 ([`작업 진행 규칙.md §6.1`](작업%20진행%20규칙.md)) |
```
...
```markdown
| docs 구조 근본 재설계 | meta | docs, policy-redesign, archive, wiki-integration, simplification | [meta/docs-structure-redesign](meta/docs-structure-redesign/) | 진행 | 2026-05-15 | ← [_archive/meta/archive-modules-index](_archive/meta/archive-modules-index/) (사용자 본질 재검토) | 4가지 변화(A 어카이브 폐지·B 진행상황/이력 통합·C list 단일표·D 영속 콘텐츠 Wiki 통합) 정책 + 지침 5개 갱신 (25곳). Task β·γ 후속 |
```
...
git add docs/지침/*.md docs/지침/일반/*.md CLAUDE.md "docs/회고/작업 진행.md" docs/tasks/list.md docs/tasks/meta/docs-structure-redesign/
git status -s   # 변경 확인
...
1. `이력.md` 작성 (작업 폴더에) — 현 형식 (기간·병합·요약)
2. `진행상황.md` 마지막 갱신 (완료 표기)
3. `git mv docs/tasks/meta/docs-structure-redesign/ docs/tasks/_archive/meta/docs-structure-redesign/`
4. `docs/tasks/list.md` 활성 → 완료 이동 + 갱신 이력 한 줄
5. 루트 `_modules.md` `meta` 행 자기 반영 (작업 수 9 → 10, 최근 작업 `docs-structure-redesign`, 최종 업데이트 2026-05-15)
...
| Task | 진입 조건 | 범위 |
|---|---|---|
| Task β | Task α 머지 완료 후 별도 작업으로 시작. 신규 작업 폴더 `meta/tasks-structure-migration` 또는 동등 | 5 repo 25 작업 `_archive/` → 활성 폴더 복귀 + 진행상황·이력 통합 + list.md 단일표 재작성 + `_modules.md` 위치 이전 |
| Task γ | Task α 머지 완료 후 별도 작업. 신규 폴더 `meta/permanent-docs-to-wiki` 또는 동등 | 5 repo `사용방법/`·`참고/` → Wiki Vault 이전 + 폐지 + 위키링크 재배선 |

본 task α 종료 보고 시 두 후속 작업 안내 (`이력.md` §후속 영향에 명시).

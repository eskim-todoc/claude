...
**TL;DR**: 5 repo 각각 동일 패턴 — ① archive 폴더들 활성 위치로 `git mv` ② `_archive/_modules.md` → `tasks/_modules.md` ③ 24 진행상황.md → 이력.md 시계열 로그 흡수 + 진행상황 `git rm` ④ list.md 단일표 재작성 + _modules.md 작업 단위 2층 확장 ⑤ 외부 archive 인용 2곳 갱신 ⑥ feature·머지·push. 응답 5개로 분할 진행.
...
flowchart TD
    ...
    B["응답 2: 5 repo archive → 활성 git mv (일괄 Bash) + _modules.md 위치 이전"]
    B --> C["응답 3: 24 진행상황.md → 이력.md 시계열 로그 흡수 + git rm"]
    C --> D["응답 4: 5 repo list.md 단일표 재작성 + _modules.md 작업 단위 2층 + 외부 인용 + 5 repo Git"]
...
# Note: $dir는 _archive/meta/wiki-bootstrap/ 같은 형태
...
# 2. _archive/_modules.md → tasks/_modules.md
[ -f docs/tasks/_archive/_modules.md ] && git mv docs/tasks/_archive/_modules.md docs/tasks/_modules.md
...
### 3.1 각 진행상황.md → 이력.md 시계열 로그 섹션 흡수
...
# 각 작업 폴더에서:
# - 진행상황.md 본문 읽기
# - 이력.md에 ## 단계 진척 + ## 시계열 로그 섹션 append (기존 본문 위에)
# - 진행상황.md 삭제
...
## 완료 요약
(기존 이력.md 본문 — 기간·병합·무엇을·왜·후속)
```

### 3.3 진행상황 없는 9 작업 — 흡수 없이 이력.md만 유지

- 루트: `wiki-bootstrap` (이미 통합되어 있을 수도)
- Sound1: 8 작업 (Task α 이전 archive 작업 일부)

해당 작업은 시계열 로그·단계 진척 섹션 추가 없이 이력.md 그대로. frontmatter `tags`에 `상태:완료` 만 추가.
...
|---|---|---|---|---|---|---|---|
| ... | ... | ... | 완료 | 2026-04-24 ~ 2026-04-24 | [meta/docs-restructure](meta/docs-restructure/) | ... | ... |
| ... | ... | ... | 진행 | 2026-05-15 | [meta/tasks-structure-migration](meta/tasks-structure-migration/) | ← Task α | ... |
...
cd <repo>
git checkout -b claude_feature_tasks-structure-migration
# 위 §2 git mv, §3 진행상황 흡수, §4 list.md, §5 _modules.md, §6 외부 인용
git add -A
...
1. 본 task `진행상황.md`를 `이력.md`의 `## 시계열 로그` 섹션에 흡수 + `git rm 진행상황.md`
2. 본 task `이력.md`에 `## 완료 요약` 섹션 추가 (기간·병합·무엇을·왜·후속)
3. 루트 `list.md` (Task β 변환 후 단일표)에서 본 task 행 `상태` 컬럼을 `완료`로 변경 + `완료일` 기록
4. 루트 `_modules.md`의 `meta` 모듈 작업 수 갱신 + 작업 상세 표에 본 task 추가
...
| β-5 _modules.md 위치 | `ls 5 repo/docs/tasks/_modules.md` | 모두 존재 |
...
| `_modules.md` 작업 단위 2층 (직전 합의 옵션 A) | ✅ §5에서 적용 |
...
| _modules.md 작업 단위 2층 본문 (5 repo 데이터) | 응답 4에서 정확 작성 |

...
**TL;DR**: 5 repo 각각 동일 패턴 — ① archive 폴더들 활성 위치로 `git mv` ② `_archive/모듈 현황.md` → `tasks/모듈 현황.md` ③ 24 진행상황.md → 이력 및 결과.md 시계열 로그 흡수 + 진행상황 `git rm` ④ 작업 목록.md 단일표 재작성 + 모듈 현황.md 작업 단위 2층 확장 ⑤ 외부 archive 인용 2곳 갱신 ⑥ feature·머지·push. 응답 5개로 분할 진행.
...
flowchart TD
    ...
    C --> D["응답 4: 5 repo 작업 목록.md 단일표 재작성 + 모듈 현황.md 작업 단위 2층 + 외부 인용 + 5 repo Git"]
    D --> E["종료 정리 (응답 7과 통합 — γ 함께)"]
```

## 2. 단계별 상세 — repo별 git mv 일괄 처리
...
**TL;DR**: <repo>의 `tasks/` 하위 모듈별 최종 업데이트 + 작업 누적 인덱스.
...
루트(10), Sound1(20), auto-rtt-viewer(2), ez8300-study(1), sound1-fw-extractor(1) 모두 채워야 함.
...
cd <repo>
git checkout -b claude_feature_tasks-structure-migration
# 위 §2 git mv, §3 진행상황 흡수, §4 작업 목록.md, §5 모듈 현황.md, §6 외부 인용
git add -A
...
3. 루트 `작업 목록.md` (Task β 변환 후 단일표)에서 본 task 행 `상태` 컬럼을 `완료`로 변경 + `완료일` 기록
...
| β-4 작업 목록.md 단일표 | `Read 5 repo/docs/tasks/작업 목록.md` | 단일 표, status 컬럼 |
...
- 본 Task β가 새 구조 첫 사용 사례

## 4. 위험 관리

| 위험 | 가능성 | 영향 | 완화 |
|---|---|---|---|
| 한글 path 처리 실패 | 중 | 중 | PowerShell 전환 |
| 5 repo 동시 처리 시간 | 높음 | 중 | 응답 5개 분할 |
| 작업 목록.md 자동 변환 실패 | 중 | 낮음 | Sound1은 별도 Edit |
| 모듈 현황.md 작업 단위 2층 미반영 | 중 | 낮음 | 후속 작업으로 분리 |

## 5. 작업 분담 (서브에이전트 활용)

- **메인 agent**: 5 repo 모든 마이그레이션·Git
- **서브에이전트 (`Explore`)**: Plan Mode 단계의 외부 archive 인용 전수 식별 (수행됨)
- **병렬 호출**: 5 repo Git은 순차 (한 응답에 묶음)

## 6. 진행 추적

- [x] 응답 2: 5 repo archive git mv + 모듈 현황.md 위치 이전
- [x] 응답 3: 24 진행상황.md 흡수 + git rm
- [x] 응답 4: 작업 목록.md 단일표 + 모듈 현황.md 2층 + 외부 인용 + 5 repo Git
- [x] 응답 7: 종료 정리 (γ와 통합)
...
| 지침 | 준수 |
|---|---|
| Task α 갱신본 신 구조 | 완료 — 본 task가 신 구조 첫 사용 사례 |
| Git scope 분리 | 완료 — 5 repo 각각 feature·머지·push |
| `모듈 현황.md` 작업 단위 2층 (직전 합의 옵션 A) | 완료 — §5에서 적용 |

### 10.2 논리적 정합성

| 점검 | 결과 |
|---|---|
| Task α의 정책이 본 task에 정확 반영 | 완료 — A·B·C 모두 본 task 구현. D는 Task γ |
| 본 task 자기 적용 — 새 구조 첫 사용 사례 | 완료 — 종료 시점 처리 §8에 명시 |
| Task γ 영역 충돌 없음 | 완료 |
...
| 5 repo 작업 목록.md 정확한 본문 (각 작업 행) | 응답 4에서 정확 변환 |

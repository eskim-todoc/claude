...
**TL;DR**: 사용자가 엄브렐러(`E:\workspace\`) 아래 `projects/<이름>/`으로 프로젝트를 git clone한 뒤 **그 프로젝트 폴더에서 세션을 열고** 자유 표현 트리거("새 프로젝트 생성", "기본 세팅" 등)를 쓰면 ① 진단 → ② CLAUDE.md(루트 헌법을 절대경로로 참조하는 슬림 패턴) → ③ 표준 폴더 풀 세트 → ④ Git 처리(그 프로젝트 repo만)의 4단계로 처리한다. **rules 저장소는 관여하지 않는다**(중앙 레지스트리 폐지, 2026-07-13~). **본 절차 적용은 4단계 작업 진행 면제**(정형 반복).
...
> [!IMPORTANT]
> `.claude/settings.json`에 `{"worktree":{"bgIsolation":"none"}}`를 둔다. 백그라운드(claude agents) 세션이 각 프로젝트의 메인 체크아웃을 워크트리 격리 없이 **직접 편집**할 수 있게 하는 설정이며, **repo별 설정이라 프로젝트마다 필요**하다. (설정 파일이 이미 있으면 `worktree` 키만 병합.)

> [!NOTE]
> `사용방법/`·`참고/` 폴더는 표준에서 **제외**. 도구·CLI 매뉴얼·외부 스펙·도메인 지식은 Wiki Vault(형제 repo `E:\workspace\projects\wiki\`, 신규 프로젝트라면 해당 도메인 폴더를 새로 만들거나 기존 도메인에 편입)에 직접 작성한다 (영속 콘텐츠 단일 출처 정책, Task α 결정. 2026-07-08 도메인 재편 이후 배치 기준은 `E:\workspace\projects\wiki\README.md` 참조).
...
- [`문서 작성 규칙.md`](문서%20작성%20규칙.md) — 폴더 구조·frontmatter (하위 [`문서/폴더-구조.md`](문서/폴더-구조.md)·[`문서/메타-frontmatter.md`](문서/메타-frontmatter.md))

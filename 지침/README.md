...
|  | [`일반/서브에이전트 활용.md`](일반/서브에이전트%20활용.md) | core | 병렬·효율 — 작업 패턴별 매핑·병렬 호출·안티패턴 | 작업 계획 수립 시 |
|  | [`일반/워크플로우 오케스트레이션.md`](일반/워크플로우%20오케스트레이션.md) | experimental | 4대 패턴·검증 루프·fan-in 예산·폭주 서킷브레이커 | 다노드 fan-out 시 |
|  | [`일반/에이전트 로그 구조.md`](일반/에이전트%20로그%20구조.md) | experimental | 에이전트-로그 3종 기록(오케 사고과정·노드 입출력 원문·위임 설계 근거) 규약·양식 | 서브에이전트 fan-out 시 |

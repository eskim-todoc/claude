...
**TL;DR**: rules와 각 프로젝트는 형제 독립 repo이며 **세션이 열린 저장소(cwd) 하나가 작업 대상 (§0)**. `main`은 사용자만 직접 관리. Claude는 `claude_develop`에서 `claude_feature_*` 임시 브랜치로 작업 → `--no-ff` 병합 (사용자 승인 후) → 기능 브랜치 삭제.

## 0. 작업 스코프 판단 (선행 단계)

rules와 각 프로젝트는 엄브렐러(`E:\workspace\`) 아래 **형제 독립 repo**다. 브랜치 작업을 시작하기 전, **세션이 열린 저장소(cwd)가 곧 작업 대상 repo**임을 확인한다 — 그 repo 하나에 대해서만 커밋·병합한다. 다른 repo 작업이 필요하면 그 폴더에서 별도 세션을 연다.

```mermaid
flowchart TD
    S[작업 시작] --> Q{세션 cwd?}
    Q -->|E:/workspace/rules| R[rules repo<br/>claude_* 브랜치]
    Q -->|E:/workspace/projects/&lt;이름&gt;| P[해당 프로젝트 repo<br/>claude_* 브랜치]
    R --> OK["§1 브랜치 작업 진행"]
    P --> OK["§1 브랜치 작업 진행"]
...
> [!IMPORTANT]
> 상세: [`브랜치, 병합 규칙.md` §작업 스코프](브랜치,%20병합%20규칙.md#작업-스코프-한-세션--한-repo)
...
1. **기능 브랜치 생성 & 체크아웃**
   ...
   ```
2. **작업 & 커밋** — 메시지 형식은 [`커밋 메시지.md`](커밋%20메시지.md) 참조 (Prefix 1~2개 + 무엇·왜·어떻게)
   ```bash
   git add <files>
...
### 8.1 Git 브랜치 워크플로우 (2026-04-17)

rules repo 및 각 프로젝트 repo 공통 브랜치 모델(§1 참조). 작업 완료 흐름: 커밋 → 사용자 확인 → `--no-ff` 병합 → 브랜치 삭제. 단순 조회/질문은 브랜치 생성 없이 가능.

### 8.2 병합 전 사용자 승인 필수 (2026-04-17)
...
### 8.3 `.gitignore` whitelisting + `git rm --cached` 머지 사이클 함정 (2026-05-08)
...
### 8.4 클론 후 claude_develop 자동 전환 (2026-04-17)
...
### 8.5 묵혀 둔 미머지 원격 브랜치 회수 — 사용자 회상 트리거 (2026-05-14)

사용자가 *"전에 ~ 한 적 있지 않아? 다른 PC였는지 기억 안 난다"* 류 자연어 회상으로 잊혀진 작업물을 떠올리면, 활성 `tasks/작업 목록.md`에서 안 보일 가능성이 크다 — main 머지 안 된 채 `claude_*` 또는 `claude_docs_*` 브랜치로 origin 원격에만 남아 있을 수 있기 때문.
...
### 8.6 Bash append+git rm 원자성 — 보이지 않는 누적 부작용 (2026-05-15)

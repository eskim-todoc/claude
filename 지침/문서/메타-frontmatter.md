...
- **필수 적용**: 모든 영속 `.md` 문서. 구체적으로:
  ...
  - 폴더별 `README.md` (자체 인덱스 성격이지만 일관성 우선)
  - 루트 진입점 `CLAUDE.md` (루트·프로젝트 양쪽)
- **미적용**: `scratch/` 등 git outside 임시 `.md`만 예외
...
- **필수**: `지침/`, `사용방법/`, `참고/`
- **선택**: 서브폴더(`지침/일반/`, `지침/Git/`, `지침/코딩/`, `지침/일반/문서/`, `지침/일반/작업/` 등) — 5+ 파일 시 가치
...
- **중복 회피**: CLAUDE.md는 `docs/지침/` 폴더의 존재만 표시. 그 안 파일 인덱스는 `docs/지침/README.md`가 담당.

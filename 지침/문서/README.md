...
**TL;DR**: '문서가 어떤 형태여야 하는가'를 규정하는 지침의 단일 홈. 작성 규칙(개념)·폴더 구조·메타(frontmatter)·작성 스타일·테마 토큰·테마 3트랙·산출물 양식 5종을 한곳에 모았다. **본 표가 문서/ 폴더의 유일 라우팅 인덱스** — 신규 파일 추가 시 이 표만 갱신하고, 상위 `../README.md`는 문서/ 카테고리 1행만 유지한다.
...
| [양식/](양식/) | 요구사항(인터뷰 기록)·계획(분석 흡수·게이트 1 체크박스)·검증리스트(2열)·이슈보고서·이력 및 결과 표준 템플릿 (복사해 사용) |
